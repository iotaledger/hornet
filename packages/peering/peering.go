...
func NewManager(opts Options, peers ...*config.PeerConfig) *Manager {
	m := &Manager{
		Events: Events{
			ConnectedAutopeeredPeer:               events.NewEvent(peer.PeerCaller),
			PeerHandshakingIncoming:               events.NewEvent(events.StringCaller),
			PeerConnected:                         events.NewEvent(peer.PeerCaller),
			PeerDisconnected:                      events.NewEvent(peer.PeerCaller),
			PeerRemovedFromReconnectPool:          events.NewEvent(peer.PeerCaller),
			PeerMovedIntoReconnectPool:            events.NewEvent(peer.OriginAddressCaller),
			PeerMovedFromConnectedToReconnectPool: events.NewEvent(peer.PeerCaller),
			PeerHandshakingOutgoing:               events.NewEvent(peer.PeerCaller),
			Reconnecting:                          events.NewEvent(events.Int32Caller),
			ReconnectRemovedAlreadyConnected:      events.NewEvent(peer.PeerCaller),
			AutopeeredPeerHandshaking:             events.NewEvent(peer.PeerCaller),
